...
export default function Members() {
    ...
    const handleCategoryFilter = (cat: string) => {
        setCategory(cat);
        setPage(1);
        fetchMembers({ category: cat || undefined, search: search || undefined });
    };
            ...
            <div className="bg-card rounded-xl border border-border shadow-card overflow-hidden">
                {isLoading ? (
                    ...
                    <div className="py-16 text-center">
                        <p className="text-muted-foreground text-sm">No members found matching your filters.</p>
                    </div>
                ) : (
                            ...
                            <tbody>
                                {paged.map(m => (
                                    <tr key={m.member_id}>
                                        ...
                                        <td>
                                            <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${m.payment_status === 'paid' ? 'bg-success/10 text-success' : 'bg-yellow-500/10 text-yellow-600'}`}>
                                                {m.payment_status}
                                            </span>
                                        </td>
                                        ...
                                        <td>
                                            <div className="flex items-center gap-1">
                                                ...
                                                {m.registration_status !== 'approved' && (
                                                    <button
                                                        onClick={(e) => handleApprove(m.member_id, e)}
