...
import { ArrowLeft, Mail, CheckCircle, Clock, FileText, Phone, Tag, MapPin, ExternalLink, Download } from 'lucide-react';
...
function FileLink({ filePath, label }: { filePath?: string; label: string }) {
    ...
    return (
        <a href={url} target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border text-xs font-medium text-primary hover:bg-accent/50 transition-colors">
...
export default function MemberProfile() {
    ...
    const { approveMember, logs } = useData();
    ...
    useEffect(() => {
        const fetchMember = async () => {
            try {
                setIsLoading(true);
                const res: any = await api.get(`/members/${id}`);
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            setMember(prev => prev ? { ...prev, registration_status: 'approved' } : null);
        } finally {
            setApproving(false);
        }
    };
                    ...
                    <div className="bg-card rounded-xl border border-border shadow-card p-5">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-start gap-3">
                                ...
                                <div><p className="text-xs text-muted-foreground">Email</p><p className="text-sm">{member.email}</p></div>
                            </div>
                            <div className="flex items-start gap-3">
                                <Phone size={14} className="text-muted-foreground shrink-0 mt-0.5" />
                                <div><p className="text-xs text-muted-foreground">Phone</p><p className="text-sm">{member.phone}</p></div>
                            </div>
                            {member.address && (
                                <div className="flex items-start gap-3">
                                    <MapPin size={14} className="text-muted-foreground shrink-0 mt-0.5" />
                                    <div><p className="text-xs text-muted-foreground">Address</p><p className="text-sm">{member.address}, {member.state}, {member.country}</p></div>
                                </div>
                            )}
                            <div className="flex items-start gap-3">
                                <Tag size={14} className="text-muted-foreground shrink-0 mt-0.5" />
                                <div><p className="text-xs text-muted-foreground">Category</p><p className="text-sm capitalize">{member.category}</p></div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    {details && (
                        <div className="bg-card rounded-xl border border-border shadow-card p-5">
                            ...
                            {(member.studentDetails?.student_id_card_file ||
                                ...
                                member.graduateDetails?.cv_file ||
                                member.professionalDetails?.cv_file ||
                                member.organizationDetails?.company_certificate_file) && (
                                    <div className="mt-4 pt-4 border-t border-border">
                                        <p className="text-xs font-semibold mb-2">Uploaded Documents</p>
                                        <div className="flex flex-wrap gap-2">
                                            <FileLink filePath={member.studentDetails?.student_id_card_file} label="Student ID Card" />
                                            <FileLink filePath={member.graduateDetails?.certificate_file} label="Certificate" />
                                            <FileLink filePath={member.graduateDetails?.cv_file} label="CV" />
                                            <FileLink filePath={member.professionalDetails?.cv_file} label="ISO Document" />
                                            <FileLink filePath={member.organizationDetails?.company_certificate_file} label="Company Certificate" />
                                        </div>
                                    </div>
                                )}
                        </div>
